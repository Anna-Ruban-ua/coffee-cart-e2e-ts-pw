...
test('Check cart reset after successful purchase', async ({ page }) => {
  ...
  await page.locator('[class="cup-body"][data-cy="Espresso"]').click();
...
test('Check the extra cup proposal is displayed and matching the design', async ({ page }) => {
  await page.goto('');
  await page.locator('[class="cup-body"][data-cy="Espresso"]').click();
  await page.locator('[class="cup-body"][data-cy="Flat-White"]').click();
  await page.locator('[class="cup-body"][data-cy="Espresso-Macchiato"]').click();
  await expect(page.locator('.promo>span')).toContainText('It\'s your lucky day! Get an extra cup of Mocha for $4.');
  await expect(page.locator('[class="cup-body disabled-hover"]')).toBeVisible();
...
test('Check that user can accept extra cup proposal and total increases for 4$', async ({ page }) => {
  await page.goto('');
  await page.locator('[class="cup-body"][data-cy="Cafe-Latte"]').click();
  await page.locator('[class="cup-body"][data-cy="Espresso-Con Panna"]').click();
  await page.locator('[class="cup-body"][data-cy="Americano"]').click();
